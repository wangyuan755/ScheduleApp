...
public class CourseIntervalSet {
    ...
    public void setSemesterLength(int semesterLength) {
        ...
        timestamp.setEndTime((long) semesterLength *7*5);
    }
    ...
    public void insertCourse(long ID,String CourseName , String Teacher,String Location,int creditHour) throws NumberFormatException
    {
        if(creditHour<=0)
        {
            throw new NumberFormatException("学时数应大于0");
        }
        if(creditHour%2 !=0)
        {
            throw new NumberFormatException("学时数应为偶数");
        ...
        for(Course i:CourseList.keySet())
        {
            ...
            {
                throw new NumberFormatException("课程名或课程ID已存在");
            }
        }
    ...
    public void addSchedule(Course course,int day,int CourseHour) throws RuntimeException
    {
        int time = timeTransfer(day,CourseHour);
        int remain = CourseList.get(course);
        if(remain==0)
        {
            throw new RuntimeException("No remain time");
        }
...
//TODO
    ...
    public  ArrayList<Integer> getCourseTime(long ID)
    {
        ArrayList<Integer> res = new ArrayList<>();
        for(int i = 0;i<timestamp.size();i++)
        {
            Item<Course> item = timestamp.getByIndex(i);
            if (ID == item.getLabel().getCourseID())
            {
                res.add((int)item.getStart());
            }
        }
        return res;
    }
